package halloboard.api.detector 
{
	...
	/**
	 * //	
	 * var detector:ImageObjectDetector = new ImageObjectDetector();
		...
		{			
			...
			GenericFlow.progress(function(next:Function):void {
								...
								function(outputRects:Array):void {
									...
									checkTime = (getTimer() - checkTime) / 1000;
									trace("Xử lý trong: " + checkTime + " giây");
									if (callBack != null) {
										callBack(_this);
									}
									trace("end");
				...
				for (var i:int = 0; i < _pieceInfos.length; i ++) {
					if (_pieceInfos[i]) {
						...
						if (piece.bmpd) {
							...
							bmdtList.push(piece.bmpd as BitmapData);
						}
					}
				}
				//
